        ...
        <Transition name="slide">
          <div v-if="showMenu" class="flex flex-col gap-2">
            <UTooltip text="Resetear factura" side="right">
              <UButton icon="i-lucide-trash-2" color="red" variant="soft" class="rounded-full" @click="resetInvoice" />
            </UTooltip>
            <UTooltip text="Cambiar Color" side="right">
              <UButton icon="i-lucide-palette" color="purple" variant="soft" class="rounded-full" @click="changeColor" />
            </UTooltip>
          </div>
        </Transition>
      </div>
        ...
        <div class="flex justify-between items-start mb-12">
          <div class="flex-1 space-y-1">
            ...
            <div class="max-w-xs space-y-1">
              <UInput v-model="invoice.sender.name" placeholder="Your Company*" variant="none" class="font-bold text-lg p-0 focus:ring-0" />
              ...
              <UInput v-model="invoice.sender.address" placeholder="Company Address" variant="none" class="p-0 focus:ring-0" />
              <UInput v-model="invoice.sender.cityStateZip" placeholder="City, State ZIP" variant="none" class="p-0 focus:ring-0" />
              <UInput v-model="invoice.sender.country" placeholder="Country" variant="none" class="p-0 focus:ring-0" />
              <UInput v-model="invoice.sender.phone" placeholder="Phone No*" variant="none" class="p-0 focus:ring-0" />
              <UInput v-model="invoice.sender.email" placeholder="Email Address*" variant="none" class="p-0 text-primary-600 focus:ring-0" />
            </div>
          </div>
        ...
        <div class="flex justify-between items-start mb-12">
          <div class="flex-1 space-y-1">
            <UInput 
              ...
              variant="none" 
              class="font-bold text-gray-400 text-sm mb-2 uppercase tracking-widest p-0 focus:ring-0 w-full" 
            />
            ...
            <div class="max-w-xs space-y-1">
              ...
              <UInput v-model="invoice.client.country" placeholder="Country" variant="none" class="p-0 focus:ring-0" />
              <UInput v-model="invoice.client.email" placeholder="Client's Email" variant="none" class="p-0 focus:ring-0" />
            </div>
          </div>
          ...
          <div class="w-64 space-y-3 text-right">
            <div class="flex items-center justify-end gap-4">
              ...
              <UInput v-model="invoice.number" placeholder="####" variant="none" class="w-24 text-right font-mono focus:ring-0" />
            </div>
            <div class="flex items-center justify-end gap-4">
            ...
            <div class="flex items-center justify-end gap-4">
              ...
              <UInput v-model="invoice.dueDate" type="date" variant="none" class="w-32 text-right focus:ring-0" />
            </div>
          </div>
        </div>
        ...
        <div class="mb-8">
          ...
          <div v-for="(item, index) in invoice.items" :key="index" class="grid grid-cols-12 border-b border-gray-100 p-4 items-center group relative hover:bg-gray-50 transition-colors">
            ...
            <div class="col-span-2 text-center">
              <UInput v-model.number="item.quantity" type="number" variant="none" class="text-center font-bold focus:ring-0" />
            </div>
            <div class="col-span-2 text-right">
              <UInput v-model.number="item.price" type="number" variant="none" class="text-right focus:ring-0" prefix="$" />
            </div>
            
            <UButton v-if="invoice.items.length > 1" icon="i-lucide-trash" color="red" variant="ghost" size="xs" class="absolute -right-10 opacity-0 group-hover:opacity-100 transition-opacity" @click="invoice.items.splice(index, 1)" />
          </div>
          
          <UButton icon="i-lucide-plus" variant="ghost" class="mt-4 text-gray-400 hover:text-primary-600" @click="addItem">Add another line item</UButton>
        </div>

        <div class="grid grid-cols-2 gap-20 items-end">
          <div class="border border-gray-200 rounded-sm p-6 min-h-[160px] relative">
            <p class="absolute -top-3 left-4 bg-white px-2 text-xs font-bold text-gray-400 uppercase tracking-tighter">Notes:</p>
          ...
          <div class="space-y-4">
            ...
            <div class="flex justify-between items-center text-gray-600 border-b border-gray-100 pb-4">
              ...
              <div class="flex items-center gap-2">
                <UInput v-model.number="invoice.taxPercent" type="number" size="xs" class="w-20 text-right focus:ring-0" variant="none" />
            ...
            <div class="flex justify-between items-center text-gray-600 border-b border-gray-100 pb-4">
              <span>Discount:</span>
              <div class="flex items-center gap-2">
                <UInput v-model.number="invoice.discountPercent" type="number" size="xs" class="w-20 text-right focus:ring-0" variant="none" />
                <span class="font-bold">%</span>
              </div>
            </div>
            ...
            <div class="flex justify-between text-2xl font-bold pt-2 border-t-2 border-gray-900">
              ...
              <span class="text-primary-600 font-serif">${{ total.toFixed(2) }}</span>
            </div>
          </div>
        </div>
        
        <div class="mt-12 flex justify-end">
          <UButton size="xl" color="black" icon="i-lucide-download" @click="generatePDF">Generate PDF</UButton>
        </div>
      </UCard>
    </UContainer>
  </UApp>
...
const invoice = reactive({
  ...
  client: { 
    sectionTitle: "CLIENT'S COMPANY", // Campo editable añadido
    firstName: '', 
    lastName: '', 
    address: '', 
    cityStateZip: '', 
...
const changeColor = () => {
  const colors = ['#000000', '#9333ea', '#2563eb', '#dc2626']
...
//   } catch (error) {
//     alert("Error conectando con Laravel")
//   }
// }
const generatePDF = async () => {
  const { public: { apiBase } } = useRuntimeConfig() 

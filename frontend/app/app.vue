          ...
          <div class="space-y-4">
            ...
            <div class="flex justify-between items-center text-gray-600 border-b border-gray-100 pb-4">
              ...
              <div class="flex items-center gap-2">
                <UInput v-model.number="invoice.taxPercent" type="number" min="0" size="xs" class="w-20 text-right focus:ring-0 font-bold" variant="none" />
            ...
            <div class="flex justify-between items-center text-gray-600 border-b border-gray-100 pb-4">
              <span>Discount:</span>
              <div class="flex items-center gap-2">
                <UInput v-model.number="invoice.discountPercent" type="number" min="0" size="xs" class="w-20 text-right focus:ring-0 font-bold" variant="none" />
                <span class="font-bold">%</span>
              </div>
            </div>
...
// Lista de 10 colores para elegir
const colors = [
  '#000000', // Black
  '#1e40af', // Blue
  '#065f46', // Emerald
...
const handlePasteNumbers = (e) => {
  ...
  if (/[^0-9]/.test(pasteData)) {
    e.preventDefault();
    const cleanValue = pasteData.replace(/\D/g, '').slice(0, 10);
    invoice.sender.phone = cleanValue;
  }
}
...
const generatePDF = async () => {
  if (invoice.sender.phone.length !== 10) {
    alert("Error: El teléfono debe tener 10 dígitos.");
    return;
  }
  if (!invoice.sender.email.includes('@') || (invoice.client.email && !invoice.client.email.includes('@'))) {
    alert("Error: Revisa los correos electrónicos.");
    return;
  }
